...
Cube& RubiksCube::operator () (int i, int j, int k)
{
    return c(i, j, k);
}

void RubiksCube::load(std::string filename)
{
    ...
    file.close();
    std::cout << "Cube successfully loaded from file.\n";
}

void RubiksCube::save(std::string filename)
{
    ...
    file.close();
    std::cout << "Cube successfully loaded to file.\n";
}
...
std::string RubiksCube::colors_to_string(std::vector<char>& colors)
{
...
bool RubiksCube::color_opposite(char a, char b)
{
    std::vector<char> all_colors = {'y', 'b', 'r', 'o', 'g', 'w'};
    for (int i = 0; i < SIDES_COUNT; i++)
    {
        if (a == all_colors[i])
        {
...
bool RubiksCube::check_invariation()
{
    ...
    for (int i = 0; i < SIDE_LENGHT; i++)
    {
        for (int j = 0; j < SIDE_LENGHT; j++)
        {
            for (int k = 0; k < SIDE_LENGHT; k++)
            {
                std::vector<char> colors = standart_cube(i - 1, j - 1, k - 1).getColors();
                if (colors.size() == 0)
                    continue;
                std::string s = colors_to_string(colors);
                components[s] = 1;
            }
        }
    }
    ...
    for (int i = 0; i < SIDE_LENGHT; i++)
    {
        for (int j = 0; j < SIDE_LENGHT; j++)
        {
            for (int k = 0; k < SIDE_LENGHT; k++)
            {
                std::vector<char> colors = c(i - 1, j - 1, k - 1).getColors();
                if (colors.size() == 0)
                    continue;
                std::string s = colors_to_string(colors);
...
void RubiksCube::rotation_helper(char axis, int value_axis, bool inv)
{
...
void RubiksCube::motion(std::string s, bool check_rotate)
{
    ...
    steps.push_back(RotationAnimation(axis, value_axis, inv, s));
}

void RubiksCube::motion(std::string s)
{
    motion(s, true);
}

void RubiksCube::rotation(std::string s, bool check_rotate)
{
    bool inv = false;
    char axis;
    if (s == "up")
    ...
    steps.push_back(RotationAnimation(axis, 2, inv, s));
}

void RubiksCube::rotation(std::string s)
{
    rotation(s, true);
}

void RubiksCube::generate_valid_cube()
{
    c = Info();
    std::srand(std::time(nullptr));
    int n = std::rand() % 50 + 25;
    for (int i = 0; i < n; i++)
    {
        int k = std::rand() % 10;
        switch (k)
...
void RubiksCube::double_function(std::string type, std::string command)
{
    ...
    if (type == "rotation")
        ...
        throw std::runtime_error("Double function call was wrong");
    for (int i = 0; i < 2; i++)
    {
        if (rotation_motion)
            motion(command);
...
void RubiksCube::cross_yellow_up()
{
    if (c(0, 1, 0)["top"] == 'y')
        return;
    for (int i = 0; i < 3; i++)
...
void RubiksCube::cross_rotate_top(int x, int z)
{
    for (int i = 0; i < 4; i++)
    {
        if (c(x, 1, z)["top"] == 'w')
...
void RubiksCube::cross_whites_up(int &counter)
{
    int k = 1;
    if (c(-1, 0, k)["front"] == 'w')
    {
...
void RubiksCube::cross_rotate_top()
{
    for (int i = 0; i < 3; i++)
    {
...
std::vector<char> RubiksCube::color_corner(int x, int y, int z, char color)
{
    std::vector<char> corner = c(x, y, z).getColors();
...
bool RubiksCube::colors_existence(std::vector<char> &corner, char a, char b)
{
...
void RubiksCube::first_layer_rotate_top()
{
    for(int i = 0; i < 3; i++)
    {
        std::vector<char> top_corner = color_corner(1, 1, 1, 'w');
...
void RubiksCube::first_later_rotate_top(char a, char b)
{
    for (int i = 0; i < 3; i++)
    {
        std::vector<char> top_corner = color_corner(1, 1, 1, 'w');
...
void RubiksCube::first_layer()
{
    for (int i = 0; i < 4; i++)
    {
        std::vector<char> corner = color_corner(1, -1, 1, 'w');
        if (corner.size() != 0)
        {
...
bool RubiksCube::colors_existence(std::vector<char> &edge, char a)
{
...
void RubiksCube::second_layer_rotate_top()
{
    for (int i = 0; i < 3; i++)
    {
        std::vector<char> edge = c(-1, 1, 0).getColors();
...
void RubiksCube::second_layer_rotate_top(char a, char b)
{
    for (int i = 0; i < 3; i++)
    {
        std::vector<char> edge = c(0, 1, 1).getColors();
...
void RubiksCube::second_layer_edge_right()
{
    pif_paf();
    rotation("right");
    pif_paf_left();
...
void RubiksCube::second_layer_edge_left()
{
    pif_paf_left();
    rotation("left");
    pif_paf();
...
void RubiksCube::second_layer()
{
    for (int i = 0; i < 4; i++)
    {
        std::vector<char> edge = c(1, 0, 1).getColors();
        bool check = colors_existence(edge, 'y');
        if (!check)
        {
            char color_front = c(0, 0, 1)["front"];
...
void RubiksCube::correct_top_cross()
{
    for (int i = 0; i < 4; i++)
    {
        int counter = 0;
...
void RubiksCube::correct_cubes_create()
{
    motion("R");
    motion("U'");
    motion("L'");
...
int RubiksCube::top_layer_correct_cubes_count()
{
    int counter = 0;
    for (int i = 0; i < 4; i++)
    {
        std::vector<char> colors = color_corner(1, 1, 1, 'y');
        char color_front = c(0, 0, 1)["front"];
        char color_right = c(1, 0, 0)["right"];
...
void RubiksCube::top_layer_rotate()
{
    for (int i = 0; i < 4; i++)
    {
        std::vector<char> color = color_corner(-1, 1, 1, 'y');
        char color_front = c(0, 0, 1)["front"];
...
bool RubiksCube::top_layer_correct_cubes()
{
