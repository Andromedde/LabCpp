...
std::vector<RotationAnimation> steps;
int iter;
int n;
...
std::vector<GLfloat> charToFloat(char i)
{
...
void translation_coords(std::vector<GLfloat> &translation, char axis, float j, float k)
{
    int n = -1;
    switch(axis)
    {
...
void display()
{
    ...
    RotationAnimation r = RotationAnimation();
    if (!animation_in_progress)
    {
        ...
        else
        {
            ...
            if (solve)
            {
                std::cout << "Rubik's cube is solved!\n";
                solve = false;
            }
            if (random)
            {
                std::cout << "A random state is set.\n";
                random = false;
                increment /= 10;
            }
        }
                ...
                if (r.axis != '0')
                {
                    ...
                    if (a == r.value_axis || r.value_axis == 2)
                    {
                        ...
                        if (!(b == c && c == 0))
                        {
                            ...
                            if (b == 0)
                                translation_coords(translation, r.axis, c * mysin(-angle, length_to_edge), c * (length_to_edge - mycos(angle, length_to_edge)));
                            else if (c == 0)
                                translation_coords(translation, r.axis, b * (length_to_edge - mycos(angle, length_to_edge)), b * mysin(angle, length_to_edge));
                            else if (b == c)
                                translation_coords(translation, r.axis, b * (length_to_edge - mycos(45 - angle, length_to_corner)), b * (length_to_edge - mysin(45 - angle, length_to_corner)));
                            else
                                translation_coords(translation, r.axis, b * (mycos(135 - angle, length_to_corner) + length_to_edge), b * (mysin(135 - angle, length_to_corner) - length_to_edge));
...
static void functions()
{
    ...
    if (user_input[0] == '/')
    {
        ...
        if (user_input == "/invariant")
        {
            bool check = cube.check_invariation();
            if (check)
            {
                RubiksCube additional_cube = cube;
                check = additional_cube.solve();
                steps.clear();
            }
            std::string s = (check) ? "correct" : "incorrect";
            std::cout << "Rubik's cube is " << s << ".\n";
        }
        ...
        {
            if (!cube.check_invariation())
            {
                std::cout << "Rubik's cube is incorrect.\n";
                return;
            }
        ...
        {
            std::string command = user_input.substr(0, position);
            std::string filename = user_input.substr(position + 1);
            std::string path = "C:/Users/Koshkin/Desktop/sss/homework6/"; //my path
            if (command == "/save")
            {
    ...
    switch (key)
    {
        ...
        default:
            if (!solve)
            {
                ...
                std::cout << user_input[user_input.length() - 1];
            }
    }
    glutPostRedisplay();
}
